...
from codeplumb.retrieve import get_context as _get_context
from codeplumb.retrieve import get_section as _get_section
from codeplumb.retrieve import resolve_reference as _resolve
...
def _emb():
    if "emb" not in _state:
        ...
        with _conn().cursor() as cur:
            ...
            cfg = cur.fetchone()
        if not cfg:
            raise RuntimeError("database not initialised; run `codeplumb init`")
...
if os.environ.get("CODEPLUMB_ENABLE_INGEST_TOOL") == "1":

    @mcp.tool()
    def ingest_document(path: str, corpus: str, layer: str = "base", profile: str = "ibc") -> dict:
        """Index a local file into a corpus (slow; writes to the database). Path must be under CODEPLUMB_INGEST_ROOT."""
        from codeplumb.ingest import ingest_document as _ingest
...
@mcp.resource("codeplumb://{corpus}/toc")
...
@mcp.resource("codeplumb://{corpus}/section/{number}")
...
@mcp.resource("codeplumb://{corpus}/document/{doc_id}")
