...
app = typer.Typer(help="codecite: bring-your-own-corpus building-code retrieval + MCP server", no_args_is_help=True)
...
def _embedder():
    ...
    with conn.cursor() as cur:
        ...
        cfg = cur.fetchone()
    if not cfg:
        typer.echo("database not initialised; run `codecite init`", err=True)
        raise typer.Exit(2)
    ...
    if emb.dimensions != cfg["dimensions"]:
        typer.echo(
            ...
            f"with {cfg['dimensions']}; refusing to mix models. Re-run `codecite init --reset`.",
            err=True,
        )
        raise typer.Exit(2)
    return conn, emb
...
def ingest(
    ...
    from codecite.ingest import AlreadyIngested, dry_run_tree, ingest_document
...
@app.command()
def corpora():
    from codecite.db import queries
    from codecite.db.pool import connect
...
def section(number: str, corpus: str | None = typer.Option(None, "--corpus"), children: bool = typer.Option(False, "--children")):
    from codecite.db.pool import connect
    from codecite.retrieve import get_section
...
def eval_cmd(
    ...
    md = render_report(results, f"codecite eval: {golden.name} ({emb.name})")
    if report:
...
def serve(transport: str = typer.Option("stdio", "--transport"), host: str = typer.Option("127.0.0.1", "--host"), port: int = typer.Option(8765, "--port")):
    ...
    from codecite.mcp_server import run
...
def fetch_oac(chapter: str = typer.Argument("4101:1"), out: Path = typer.Option(Path("./corpus/oac"), "--out")):
    ...
    from codecite.fetch.oac import fetch_chapter
