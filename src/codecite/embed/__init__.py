...
from codecite.config import Settings, get_settings
from codecite.embed.base import Embedder, FakeEmbedder
...
def get_embedder(settings: Settings | None = None, *, provider: str | None = None, model: str | None = None) -> Embedder:
    ...
    if provider == "local-st":
        from codecite.embed.local_st import LocalSentenceTransformers
    ...
    if provider in ("voyage", "openai"):
        if not s.allow_remote_embeddings:
            raise RuntimeError(
                ...
                "set CODECITE_ALLOW_REMOTE_EMBEDDINGS=1 to opt in"
            )
        from codecite.embed.remote import OpenAIEmbedder, VoyageEmbedder
